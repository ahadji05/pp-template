...
{
    ppt::Vector<float, ppt::MemSpaceHost> vA(20);
...
{
    ppt::Vector<float, memo_space> vA(10);
    ...
    float *host_data;
    ppt::MemSpaceHost::allocate(&host_data, 12);
...
TEST(test_constructor_two_explicit_host_container, GTest_Vector)
{
    ...
    ppt::Vector<double, ppt::MemSpaceHost> vA(host_data, 9);
...
{
    double host_data[9] = {0.01, -0.1, 0, 1.0, 1.1, 2.5, -12.1, 0, 2};
    ppt::Vector<double, memo_space> vA(host_data, 9);
    ASSERT_EQ(vA.get_nElems(), 9);

    double *host_data2;
    ppt::MemSpaceHost::allocate(&host_data2, 9);
    memo_space::copyToHost(host_data2, vA.get_ptr(), 9);
    for (size_t i(0); i < vA.get_nElems(); ++i)
        ASSERT_FLOAT_EQ(host_data2[i], host_data[i]);

    ppt::MemSpaceHost::release(host_data2);
}

TEST(test_copy_constructor, GTest_Vector)
{
    ppt::Vector<float, memo_space> vA(1014);
    vA.fill(1.0015);

    ppt::Vector<float, memo_space> vB(vA);
    ASSERT_EQ(vB.get_nElems(), 1014);

    float *host_data;
    ppt::MemSpaceHost::allocate(&host_data, 1014);
...
{
    ppt::Vector<float, memo_space> vA(124003);
    vA.fill(-1.0015);
    ...
    float *host_data;
    ppt::MemSpaceHost::allocate(&host_data, 124003);
