...
TEST(test_constructor_explicit_host_container, GTest_Vector) {
  ...
  vA.fill(10.5);
  for (size_t i(0); i < vA.get_nElems(); ++i) ASSERT_FLOAT_EQ(vA[i], 10.5);
}
...
TEST(test_copy_constructor, GTest_Vector) {
  ...
  for (size_t i(0); i < 1014; ++i) ASSERT_FLOAT_EQ(host_data[i], 1.0015);
...
TEST(test_copy_assign, GTest_Vector) {
  ...
  for (size_t i(0); i < 124003; ++i) ASSERT_FLOAT_EQ(host_data[i], -1.0015);
