 ...
 * iz -> 1
 *
 * After routine is applied only the value at position = iz * nx + ix should be
 * changed! (in this experiment position = 55)
 *
...
TEST(test_source_position_value, GTest_add_source) {
  ...
  ScalarField<memo_space> p(3, 50,
                            v);  // construct ScalarField p of dim: nz(3),
                                 // nx(50) with vector the underlying container

  add_source(p, 5.5, 5, 1, exec_space());  // apply routine

  float_type *host_array;
  TMP::MemSpaceHost::allocate(
      &host_array, N);  // allocate a host-array for the unit-test assertions
  ...
  ASSERT_FLOAT_EQ(host_array[1 * 50 + 5],
                  5.5);  // source should be added at this particular position
