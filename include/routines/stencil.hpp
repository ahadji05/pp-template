...
/**
 * @brief This routine calculates the second-order derivative pxx of a wavefield p using the FD scheme:
 *
 *  pxx[z,x] = -1/12*p[z,x-2] +4/3*p[z,x-1] -5/2*p[z,x] +4/3*p[z,x+1] -1/12*p[z,x+2]
 *
...
/**
 * @brief This routine calculates the second-order derivative pzz of a wavefield p using the FD scheme:
 *
 *  pzz[z,x] = -1/12*p[z-2,x] +4/3*p[z-1,x] -5/2*p[z,x] +4/3*p[z+1,x] -1/12*p[z+2,x]
 *
 * @tparam ExecSpace Execution-Space that is used to resolve the back-end implementation at compile-time.
 * @tparam MemSpace Memory-Space that must be accessible from the Execution-Space; otherwise compile-time error.
