...
template <>
void fd_pxx(ScalarField<TMP::MemSpaceHost> &pxx,
            const ScalarField<TMP::MemSpaceHost> &p,
  ...
  for (size_t iz(0); iz < nz; ++iz)
    for (size_t ix(2); ix < nx - 2; ++ix) {
      size_t i    = iz * nx + ix;
      pxx_data[i] = c2 * p_data[i - 2] + c1 * p_data[i - 1] + c0 * p_data[i] +
                    c1 * p_data[i + 1] + c2 * p_data[i + 2];
    }
}

template <>
void fd_pzz(ScalarField<TMP::MemSpaceHost> &pzz,
            const ScalarField<TMP::MemSpaceHost> &p,
            TMP::ExecutionSpaceSerial) {
  ...
  for (size_t iz(2); iz < nz - 2; ++iz)
    for (size_t ix(0); ix < nx; ++ix)
      pzz_data[iz * nx + ix] =
          c2 * p_data[(iz - 2) * nx + ix] + c1 * p_data[(iz - 1) * nx + ix] +
...
#pragma omp parallel for schedule(static, 10)
  for (size_t iz = 0; iz < nz; ++iz)
    for (size_t ix = 0; ix < nx - 2; ++ix)
      pxx_data[iz * nx + ix] = -(1 / 12) * p_data[iz * nx + ix - 2] +
                               (4 / 3) * p_data[iz * nx + ix - 1] -
                               (5 / 2) * p_data[iz * nx + ix] +
                               (4 / 3) * p_data[iz * nx + ix + 1] -
...
#pragma omp parallel for schedule(static, 10)
  for (size_t iz = 2; iz < nz - 2; ++iz)
    for (size_t ix = 0; ix < nx; ++ix)
      pzz_data[iz * nx + ix] = -(1 / 12) * p_data[(iz - 2) * nx + ix] +
                               (4 / 3) * p_data[(iz - 1) * nx + ix] -
                               (5 / 2) * p_data[iz * nx + ix] +
                               (4 / 3) * p_data[(iz + 1) * nx + ix] -
